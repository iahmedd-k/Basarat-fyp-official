...
- The PSX `*` footnote applies only to provisional purification rates; the snapshot now preserves it per company instead of marking every rate provisional. The KMI-30 table has 24 starred provisional entries and 6 unstarred/N/A entries.
...
Commits `6fb8186`, `0611785`, `e1baea9`, and `71f2653` were pushed to GitHub. The first live AWS check confirmed that screening, criteria, purification, unknown-symbol, and known non-compliant routes were already serving the new data. It caught inconsistent percentage units, which were corrected. Subsequent public smoke checks passed all assertions, including the 30-row KMI roster, exact source dates, ratio/threshold units, OGDC exception, provisional PKR 993 calculation, MEBL's missing rate, unknown symbols, HBL classification, invalid-input validation, and the purification stale-data flag. The KMI list initially timed out while waiting on the live market cache; it was decoupled from that unrelated cache and the retest passed. One rollout-period smoke run saw connection refusals; the complete retry passed. This indicates intermittent AWS availability during the rollout and should be monitored separately from endpoint data correctness.

The Shariah module serves the latest licensed, official source snapshot and identifies its data age. The snapshot itself is stale by age and some dividend purification rates are provisional; callers must show those disclosures and should not present these figures as a fresh screening against newer accounts. The Shariah endpoints and public-route behavior passed the local and AWS checks, with intermittent AWS connection refusals observed during rollout.

This does not clear the whole Stocks/Market product for production: the separate Stocks/Market audit still records stale HBL OHLCV and missing financial data, and the frontend build still depends on an unconfigured `VITE_API_URL`/hosting pipeline. The PSX redistribution license is confirmed, but the licensed feed/provider and frontend deployment configuration are not yet connected or identified.

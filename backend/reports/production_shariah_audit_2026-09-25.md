...
Commits `6fb8186`, `0611785`, and `e1baea9` were pushed to GitHub. The first live AWS check confirmed that screening, criteria, purification, unknown-symbol, and known non-compliant routes were already serving the new data. It caught inconsistent percentage units, which were corrected. A subsequent public smoke test passed all checks, including the 30-row KMI roster, exact source dates, ratio/threshold units, OGDC exception, provisional PKR 993 calculation, MEBL's missing rate, unknown symbols, HBL classification, and invalid-input validation. The KMI list initially timed out while waiting on the live market cache; it was decoupled from that unrelated cache and the retest passed. Purification responses now also expose an explicit `data_is_stale` flag.

The Shariah module serves the latest licensed, official source snapshot and identifies its data age. The snapshot itself is stale by age and some dividend purification rates are provisional; callers must show those disclosures and should not present these figures as a fresh screening against newer accounts.

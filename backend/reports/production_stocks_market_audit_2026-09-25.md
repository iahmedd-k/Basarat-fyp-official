...
Local verification after removing the fallback: **44/44** Stocks/Market tests passed across stock/market APIs, market normalization, search aliases, and OHLCV refresh. Production retesting is required after this cleanup deploys. If the licensed PSX feed does not provide fundamentals or quote OHLC, those fields should remain null until an authorized feed is identified; the user confirmed PSX market-data redistribution rights but has not yet supplied the licensed fundamentals/history provider documentation or frontend hosting details.

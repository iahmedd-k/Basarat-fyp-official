...
### Repeat production audit and frontend URL hardening (2026-09-25)

The anonymous read-only route audit was repeated against AWS after the frontend fixes. The audit runner was corrected for PowerShell 7 so HTTP 4xx responses are captured and checked as JSON instead of being misreported as script errors. All **30** route and input cases returned valid JSON and the expected status: 200 for valid requests, 422 for invalid query ranges, symbols, sort values, and bounds. The full quote request again returned **560/560** records. The repeated `search=Habib` request resolved to HBL.

The production data-quality summary still shows an old quote snapshot (`2026-09-24T23:55:11.93518+05:00`, `is_stale=true`), all 560 OHLC values unavailable/null, one null current price (AAL), 75 zero-volume records, and no arithmetic mismatches among available current/change values. No zero-volume rows appear in the top-ten losers response. HBL OHLCV history and indicators use the 2026-09-18 bar (7 days old at this check, stale). The overview day range remains present (`299.0–312.05`). HBL fundamentals remain partial with annual and quarterly statements missing. The five most extreme changes contain illiquid zero-volume names SWL, GIL, and MSOTPS, but the movers endpoint correctly filters zero-volume rows; the full quote list preserves those source rows and their prices, so consumers should use volume and freshness fields before interpreting them as current trades.

The liveness endpoint returned 200 in 0.58 seconds. Readiness returned healthy with API, database, Redis, Celery worker, and Celery beat ready, but took 8.98 seconds; monitor dependency-check latency. The 30-route probe observed up to 15.32 seconds for the 560-record full quote response.

The frontend now shares one API URL resolver between Market Pulse and News. Development still defaults to localhost; production has no localhost fallback, explicitly rejects a localhost production URL, and surfaces a configuration error when `VITE_API_URL` is missing. The production build contains no localhost API URL. `npm run build` passed; `npm run lint` passed with the same six existing warnings in `News_Seciton.jsx` and `Stockperview.jsx`. The hosting platform and API URL are still unknown, and the backend-only EC2 workflow does not deploy a frontend, so an actual production frontend connection remains unverified.

### Licensed-source cleanup (2026-09-25)

The fundamentals service had been scraping StockAnalysis.com and returning its market/fundamentals values through public API responses. StockAnalysis states it has no programmatic API and does not license its third-party data for programmatic redistribution or resale ([official API-access FAQ](https://stockanalysis.com/help/faq/api-access/); [Terms of Use](https://stockanalysis.com/terms-of-use/)). The scraper fallback and its cached-data reader were removed, and the fundamentals cache key was versioned so cached records produced by that fallback are not reused. The known-symbol company-name alias still resolves from the project's own alias list. HBL day ranges are now taken only from quote-source OHLC values; the service no longer fills a missing day range with an older history bar or an unlicensed third-party value.

Local verification after removing the fallback: **44/44** Stocks/Market tests passed across stock/market APIs, market normalization, search aliases, and OHLCV refresh. Commit `a1c1cc4` deployed successfully via [GitHub Actions](https://github.com/iahmedd-k/Basarat-fyp-official/actions/runs/36078884484). All **30** anonymous production route/input cases again returned valid JSON and expected statuses; the full quote endpoint returned 560/560 items. The `Habib` search still resolves to HBL.

The post-deployment quality pass confirms 560 stale quotes from `2026-09-24T23:55:11.93518+05:00`, all 560 OHLC values null, one null current price (AAL), 75 zero-volume rows, no arithmetic mismatches, and no zero-volume rows in the top-ten losers list. HBL history remains stale at `2026-09-18` (7 days old). HBL overview day range is now null because the licensed PSX quote source does not supply OHLC and the endpoint no longer substitutes a day range from an old bar or an unlicensed vendor. HBL fundamentals remain partial; annual and quarterly statements are null. These are visible upstream data gaps rather than fabricated values.

After deployment, **22/22** combined local Shariah API and anonymous AWS Shariah smoke checks passed. API liveness returned 200; readiness returned healthy with database, Redis and Celery checks ready, but took 5.75 seconds. Prior deployment-window connection refusals were transient; the final route suite completed successfully. Continue monitoring API availability and dependency-check latency.

The backend work and public endpoints are verified, but complete production readiness still requires the licensed provider's documentation/configuration for current quotes, historical prices and financial statements, plus the frontend hosting platform and its `VITE_API_URL`. If the licensed PSX feed does not supply those fields, they should remain null until another authorized source is connected; the user confirmed PSX market-data redistribution rights but has not yet supplied the provider/hosting details.
